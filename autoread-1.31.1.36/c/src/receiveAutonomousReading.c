...
#define usage() {errx(1, "Please provide reader URL, such as:\n"\
    "/com4 (or) /dev/ttyS0\n"\
    "-v /com4 (or) -v /dev/ttyS0 where -v enables transport listener\n");}
 ...
 *
 * @param autonomous - The reader to operate on.
 * @param b - A structure containing a pointer to the listener
 * function and a user-supplied cookie value to pass to the function
 * when called.
 */
TMR_Status
serial_addTransportListener(AutonomousReading *autonomous, TMR_TransportListenerBlock *b)
{

  b->next = autonomous->transportListeners;
  autonomous->transportListeners = b;

  return TMR_SUCCESS;
}

void
serial__notifyTransportListeners(AutonomousReading *autonomous, bool tx, 
                              uint32_t dataLen, uint8_t *data,
                              int timeout)
{
  TMR_TransportListenerBlock *block;

  block = autonomous->transportListeners;
  while (NULL != block)
  {
...
TMR_Status serial_addTransportListener(AutonomousReading *autonomous, TMR_TransportListenerBlock *block);
void serial__notifyTransportListeners(AutonomousReading *autonomous, bool tx,uint32_t dataLen, uint8_t *data, int timeout);

int main(int argc, char *argv[])
{
  TMR_Status ret;
  int i = 0;
  bool enableTransportLog = false;
  AutonomousReading autonomous;
  char readerURI[16];
  autonomous.isStatusResponse = false;
  autonomous.transportListeners = NULL;
  autonomous.elapsedTime = 0;
  ...
  for (i = 1; i < argc; i++)
  {
    // option "-v" enables transport listener
    if(0x00 == strcmp("-v",argv[i]))
    {
      enableTransportLog = true;
    }
    else if(0x00 == strncmp("/",argv[i], 1))
    {
      strcpy(readerURI,argv[i]);
    }
    else
    {
  ...
  if(enableTransportLog)
  {
    ...
    tb.cookie = stdout;
    serial_addTransportListener(&autonomous, &tb);
  }

  /* Connect to the reader */
  strcpy(autonomous.uri, readerURI);
...
do_background_receiveAutonomousReading(void * arg)
{
  ...
  while (1)
  {
    if (false == autonomous->isBasetimeUpdated)
    {
      ...
      if(autonomous->elapsedTime)
      {
        uint64_t currTime64;
        currTime64 = ((uint64_t)autonomous->readTimeHigh << 32) | autonomous->readTimeLow;
        currTime64 += (uint64_t)autonomous->elapsedTime;
      ...
      autonomous->readTimeHigh = autonomous->lastSentTagTimestampHigh = starttimeHigh;
      autonomous->readTimeLow = autonomous->lastSentTagTimestampLow = starttimeLow;
...
serial_receiveAutonomousReading(AutonomousReading *autonomous, TMR_TagReadData *trd, TMR_Reader_StatsValues *stats)
{
  ...
  if (TMR_SUCCESS == ret)
  {
    if (false == autonomous->isStatusResponse)
    {
      ...
      {
        flags = GETU16AT(autonomous->bufResponse, 9);
        autonomous->bufPointer++;
      }
      else
      {
...
serial_hasMoreTags(AutonomousReading *autonomous)
{
  ...
  if ((TMR_SUCCESS != ret) && (TMR_ERROR_TAG_ID_BUFFER_FULL != ret)
    ...
    || ((TMR_ERROR_NO_TAGS_FOUND == ret) && ((0 == msg[1]|| (0x88 == msg[5]))))))

  {
    autonomous->isBasetimeUpdated = false;
    if(0x88 == msg[5])
    {
      autonomous->elapsedTime = GETU32AT(msg, 12);
    }
    return ret;
  }
...
serial_receiveMessage(AutonomousReading *autonomous, uint8_t *data, uint8_t opcode, uint32_t timeoutMs)
{
  ...
  */
  ...
  if (NULL != autonomous->transportListeners)
  {
    serial__notifyTransportListeners(autonomous, false, inlen + receiveBytesLen, data, timeoutMs);
  }
