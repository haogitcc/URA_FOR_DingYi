...
public class SerialTransport
{
    ...
    String comPort;
    public boolean enableMultipleSelect = false;
    public static SimpleTransportListener simpleTransportListener = new SimpleTransportListener();
    protected List<SimpleTransportListener> serialListeners = new Vector<SimpleTransportListener>();
    ...
    public void receiveStreaming() throws ReaderException
    {
        ...
        while (true)
        {
            ...
            catch (Exception re)
            {
                ...
                {
                   /** If multi select option(0x88) is enabled, timestamp is also sent in the response
                    ...
                    */
                    ...
                    {
                        //Fall back to older mechanism if multiselect is not enabled, update baseTimeStamp with current system time.
                        baseTimeStamp = System.currentTimeMillis();
                    }
                }
    ...
    public void receiveMessage(int timeout, Message m) throws ReaderException
    {
        ...
        if (hasSerialListeners)
        {
          ...
          for (SimpleTransportListener l : serialListeners)
          {
  ...
  public static String convertByteArrayToHexString(byte in[])
    ...
    public static final int TAG_METADATA_NONE = 0x0000,
            ...
            | TAG_METADATA_PROTOCOL | TAG_METADATA_GPIO_STATUS | TAG_METADATA_GEN2_Q| TAG_METADATA_GEN2_LF 
            | TAG_METADATA_GEN2_TARGET;
    ...
    static
    {
        ...
        tagMetadataFlagValues.put(TagReadData.TagMetadataFlag.GEN2_Q, TAG_METADATA_GEN2_Q);
        tagMetadataFlagValues.put(TagReadData.TagMetadataFlag.GEN2_LF, TAG_METADATA_GEN2_LF);
        tagMetadataFlagValues.put(TagReadData.TagMetadataFlag.GEN2_TARGET, TAG_METADATA_GEN2_TARGET);
...
//            }
        ...
        if (t.readProtocol == TagProtocol.GEN2) 
        {
            ...
            if (meta.contains(TagReadData.TagMetadataFlag.GEN2_Q)) 
            {
                gen2.q.initialQ = m.getu8();
            }
            if (meta.contains(TagReadData.TagMetadataFlag.GEN2_LF)) 
            {
                gen2.lf = Gen2.LinkFrequency.getFrequency(m.getu8());
            }
            if (meta.contains(TagReadData.TagMetadataFlag.GEN2_TARGET)) 
            {
                switch (m.getu8()) 
                {
                    case 0x00:
 ...
 */
  ...
  /**
   * notifying transport listener
   * @param tx
   * @param data
   * @param timeout
   */ 
  static class SimpleTransportListener
  {
  ...
  /** 
   * Register a listener to be notified of serial packets.
   *
   * @param listener the SerialListener to add
   */
  public void addTransportListener(SimpleTransportListener listener)
  {
      if(null != listener)
      {
   ...
   * message packets.
   *
   * @param l the SimpleTransportListener to remove
   */
  public void removeTransportListener(SimpleTransportListener l)
  {
